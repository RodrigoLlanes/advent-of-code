...
def line_intersection(line1, line2):
    ...
    x = det(d, xdiff) / div
    y = det(d, ydiff) / div
    d1 = line1[1][2] - abs(line1[1][0] - x) - abs(line1[1][1] - y)
    d2 = line2[1][2] - abs(line2[1][0] - x) - abs(line2[1][1] - y)
...
for i in input1:
    if(i[0] == "R"):
        wire1.append((wire1[-1][0] + int(i[1:len(i)]), wire1[-1][1], wire1[-1][2] + int(i[1:len(i)])))
    if(i[0] == "L"):
        wire1.append((wire1[-1][0] - int(i[1:len(i)]), wire1[-1][1], wire1[-1][2] + int(i[1:len(i)])))
    if(i[0] == "U"):
        wire1.append((wire1[-1][0], wire1[-1][1] + int(i[1:len(i)]), wire1[-1][2] + int(i[1:len(i)])))
    if(i[0] == "D"):
        wire1.append((wire1[-1][0], wire1[-1][1] - int(i[1:len(i)]), wire1[-1][2] + int(i[1:len(i)])))
sol = 0
wire2=[(0,0,0)]
for i in input2:
    if(i[0] == "R"):
        wire2.append((wire2[-1][0] + int(i[1:len(i)]), wire2[-1][1], wire2[-1][2] + int(i[1:len(i)])))
    if(i[0] == "L"):
        wire2.append((wire2[-1][0] - int(i[1:len(i)]), wire2[-1][1], wire2[-1][2] + int(i[1:len(i)])))
    if(i[0] == "U"):
        wire2.append((wire2[-1][0], wire2[-1][1] + int(i[1:len(i)]), wire2[-1][2] + int(i[1:len(i)])))
    if(i[0] == "D"):
        wire2.append((wire2[-1][0], wire2[-1][1] - int(i[1:len(i)]), wire2[-1][2] + int(i[1:len(i)])))
