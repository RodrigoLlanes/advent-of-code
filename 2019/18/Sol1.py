...
@memoize
def solve(graph, keys, start):
    ngraph = deepcopy(graph)
    nkeys = deepcopy(keys)

    if start != "@":
        remove_vertex(ngraph, start.upper())
        nkeys.remove(start)
        if len(nkeys) == 0:
            return 0
    ...
    for (sym, dist) in edges:
        if sym.islower():
            r = solve(ngraph, nkeys, start=sym)
            if r >= 0:
                res.append(dist + r)
...
if __name__ == "__main__":
    vault_map = [list(line.strip()) for line in open("input.txt", "r").readlines()]
